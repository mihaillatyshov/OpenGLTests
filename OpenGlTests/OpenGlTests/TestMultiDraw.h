...
namespace
{
    ...
    const std::vector<Vertex2D> gObj = {
        //xy			//uv
        { 0.0f,0.1f,	0.0f,0.0f },
        { 0.2f,0.0f,	1.0f,0.0f },
        { 0.2f,0.2f,    0.5f, 0.5f},
    ...
    const std::vector<unsigned int> gObjIndex =
    {
        0,1,2,
        0,2,3,
        2,3,4
    ...
    struct MyObject
    {
        MyObject(const std::vector<Vertex2D>& vert, const std::vector<uint32_t>& ind)
            : Vertices(vert), Indices(ind) { }

        std::vector<Vertex2D> Vertices; // positions, uvs
...
MyObject obj(gObj, gObjIndex);


void GenerateGeometry()
{
    //Generate 50 quads, 50 triangles
    const unsigned num_vertices = gQuad.size() * 50 + gTriangle.size() * 50 + gObj.size();
    ...
    vMatrix[matrixIndex++] = obj.Transformation;
    //setMatrix(&vMatrix[matrixIndex++], 0.1f, 0.1f);
    for (unsigned int k(0); k != gObj.size(); ++k)
    {
        vVertex[vertexIndex++] = gObj[k];
    }
    ...
    int obj_bytes = sizeof(unsigned int) * gObjIndex.size();
    glGenBuffers(1, &gElementBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gElementBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, triangle_bytes + quad_bytes + obj_bytes, NULL, GL_STATIC_DRAW);
    ...
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, quad_bytes + triangle_bytes, triangle_bytes, gTriangleIndex.data());
...
int main()
{
    setMatrix(&obj.Transformation, 0.1f, 0.1f);
