...
namespace LM
{
	...
	SimpleObjectManager::SimpleObjectManager(BatchRenderer* renderer)
		: m_Renderer(renderer)
	{
		m_Shader = std::make_unique<Shader>("./res/Shaders/BatchRenderer/TestShader.vert", "./res/Shaders/BatchRenderer/TestShader.frag");
	...
	void SimpleObjectManager::Add(Renderable* renderable)
	{
		uint32_t FoundSlot = -1;
		int FoundMaterialSlot = 0;

		//Material material = *renderable->GetMaterial();
		std::shared_ptr<Material> materialPtr = renderable->GetMaterial(); // FIX IT!!!
		...
		for (uint32_t slotId = 0; slotId < m_Objects.size(); slotId++)
		{
			...
			{
				std::shared_ptr<Material> material = m_Materials[slotId][i];  // const or not???
		...
		// 
		//int TransformID = m_Renderer->GetTransformTree()->NowId;
		int TransformID = 0;

		//LOGI("NowId: ", m_Renderer->GetTransformTree()->NowId);
		//TransformID = 1;
		...
		//m_Transformations[FoundSlot].push_back(glm::mat4(1.0f)); // FIX IT!!!
	...
	void SimpleObjectManager::Flush()
	{
		...
		//m_Renderer->GetTransformTree()->Recalculate();
		...
		for (uint32_t i = 0; i < m_Objects.size(); i++)
		{
			...
			m_TransformTexture->SetData(&m_Transformations[i], sizeof(glm::mat4) * m_Transformations.size());
			
			//m_TransformTexture->SetData(m_Renderer->GetTransformTree()->Result.data(), sizeof(glm::mat4) * m_Renderer->GetTransformTree()->Result.size());
	...
	void BatchRenderer::PushTransform(std::shared_ptr<LM::Transformation> transformation)
	{
		auto v = *transformation;
		//m_TransformationTree.Push(transformation);
		m_TransformationTree.Push(transformation->GetTransformation(), 0, 0);
	}

	void BatchRenderer::PopTransform()
	{
		m_TransformationTree.Pop(0);
	}
	...
	{
		m_SimpleObjectManager->Add(renderable);
	}
